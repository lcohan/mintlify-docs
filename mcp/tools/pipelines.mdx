...
description: "Build, inspect, edit, and run Alphacast pipelines from natural-language prompts. Ten MCP tools for the full pipeline lifecycle — six read-only, four write — with auto-validation and auto-run after every mutation."
---

Ten tools for working with Alphacast pipelines from an MCP client. Six are read-only (inspect definitions, fetch runs, preview step output); four are write (create a pipeline, add/edit/remove steps). After every write the server auto-validates and auto-runs the pipeline, then waits for the editor run to finish so the response carries the new state.
...
List every pipeline the API key can read.

No input parameters. Returns one row per pipeline with `id`, `name`, and `repositoryId`.
